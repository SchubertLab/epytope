    ...
    def predict(self, peptides, TCRs, all: bool, trained_on: str=None):
        ...
        if isinstance(peptides, TCREpitope):
            ...
            if isinstance(peptides, list):
                ...
                raise ValueError(f"{peptides} is not of type TCREpitope or a list of TCREpitope objects")
        if isinstance(TCRs, AntigenImmuneReceptor):
            one_tcr = True
        else:
            if isinstance(TCRs, list):
                ...
                raise ValueError(f"{TCRs} is not of type AntigenImmuneReceptor or a list of AntigenImmuneReceptor "
                                 f"objects")

        
        # predict the specificity for only one TCR binding one epitope
        if one_peptide:
            if one_tcr:
                TCRs.tcr["Peptide"] = str(peptides)
                TCRs.tcr["MHC"] = peptides.mhc
                TCRs.tcr["Antigen.species"] = ""
                TCRs.tcr["Tissue"] = ""
                # create temporary csv file
                return pd.DataFrame(TCRs.tcr, index=[0])

            # predict binding specificity for one epitope to a list of TCRs
            else:
                for i, tcr in enumerate(TCRs):
                    tcr.tcr["Peptide"] = str(peptides)
                    tcr.tcr["MHC"] = peptides.mhc
                    tcr.tcr["Antigen.species"] = ""
                    tcr.tcr["Tissue"] = ""
                    data[i] = tcr.tcr
        else:
            # predict binding specificity for one TCR sequence to a list of peptides
            if one_tcr:
                for i, peptide in enumerate(peptides):
                    TCRs.tcr["Peptide"] = str(peptide)
                    TCRs.tcr["MHC"] = peptide.mhc
                    TCRs.tcr["Antigen.species"] = ""
                    TCRs.tcr["Tissue"] = ""
                    data[i] = dict(TCRs.tcr)
            else:
                if all:
                    # predict binding specificity for each TCR in TCRs to each passed epitope
                    i = 0
                    for tcr in TCRs:
                        for peptide in peptides:
                            tcr.tcr["Peptide"] = str(peptide)
                            tcr.tcr["MHC"] = peptide.mhc
                            tcr.tcr["Antigen.species"] = ""
                            tcr.tcr["Tissue"] = ""
                            data[i] = dict(tcr.tcr)
                            i += 1
                    ...
                    # predict binding specificity in the same passed order
                    if len(peptides) < len(TCRs):
                        logging.warning("There is no enough peptides")
                    for i, tcr in enumerate(TCRs):
                        tcr.tcr["Peptide"] = str(peptides[i])
                        tcr.tcr["MHC"] = peptides[i].mhc
                        tcr.tcr["Antigen.species"] = ""
                        tcr.tcr["Tissue"] = ""
                        data[i] = tcr.tcr
