...
def stitch_tcr_stapler(row, chain="TRB"):
   prefix = "beta" if chain == "TRB" else "alpha"
   tcr_bits = {"v": row[f"{chain}V_IMGT"], "j": row[f"{chain}J_IMGT"], "cdr3": row[f"cdr3_{prefix}_aa"],
               "l": "", "c": "TRBC1*01" if chain=="TRB" else "TRAC*01",
               "skip_c_checks": False, "species": species, "seamless": False,
...
def fullseq_reconstruction(stitch_tcr_func):
    try:
    ...
    try:
        ...
        df_tcrs["full_seq_reconstruct_beta_aa"] = df_tcrs.apply(lambda x: stitch_tcr_func(x, "TRB"), axis=1)

        tcr_dat, functionality, partial = fxn.get_imgt_data("TRA", st.gene_types, species)
        df_tcrs["full_seq_reconstruct_alpha_aa"] = df_tcrs.apply(lambda x: stitch_tcr_func(x, "TRA"), axis=1)
    except:
...
def stapler():
    fullseq_reconstruction(stitch_tcr_stapler)


def nettcr():
    fullseq_reconstruction(stitch_tcr_full)
    path_in = sys.argv[3]
    path_out = sys.argv[4]
